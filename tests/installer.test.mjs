...
test("dry-run shows the complete bounded install without writing", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /A local code scan/);
    assert.match(result.stdout, /Code only/);
    assert.match(result.stdout, /No email, billing dashboards, connected integrations, or traces/);
