...
// The report remains fully useful offline. This endpoint is contacted only
// after a person completes the optional contact form and explicitly consents.
const LMAB_LEAD_CAPTURE_URL = "https://api.understudylabs.com/v1/lmab/leads";
...
<section class="section" aria-labelledby="follow-up-heading"><div class="section-head"><h2 id="follow-up-heading">Want help acting on this?</h2><p class="section-note">Optional. Send your contact details to Understudy and we’ll follow up about this audit. Your repository name, code, and report findings are not sent.</p></div><form class="lead-form" id="lmab-lead-form"><label>Name<input name="name" autocomplete="name" maxlength="120" required></label><label>Work email<input name="email" type="email" autocomplete="email" maxlength="254" required></label><label>Company<input name="company" autocomplete="organization" maxlength="160" required></label><label class="lead-consent"><input name="consent" type="checkbox" required>I agree that Understudy may use these details to contact me about Lower My AI Bill.</label><button type="submit">Request follow-up</button><p class="lead-status" id="lmab-lead-status" aria-live="polite"></p></form></section>
...
const form=document.getElementById("lmab-lead-form");
const status=document.getElementById("lmab-lead-status");
form?.addEventListener("submit",async(event)=>{event.preventDefault();const button=form.querySelector("button");button.disabled=true;status.textContent="Sending…";try{const data=new FormData(form);const response=await fetch("${LMAB_LEAD_CAPTURE_URL}",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:data.get("name"),email:data.get("email"),company:data.get("company"),consent:data.get("consent")==="on",source:"lmab-report"})});if(!response.ok)throw new Error("request failed");form.reset();status.textContent="Thanks — we’ll be in touch.";}catch{status.textContent="We couldn’t send that just now. Please try again later.";}finally{button.disabled=false;}});
</script></body></html>`;
}
